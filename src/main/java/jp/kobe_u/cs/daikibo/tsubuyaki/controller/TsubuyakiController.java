...
public class TsubuyakiController {  
    ...
    //つぶやきを検索 
    @GetMapping("/search")
    String searchTsubuyaki(@ModelAttribute("kensakuForm") TsubuyakiForm form, Model model) {
        ...
        //サービスに投稿処理を依頼  
        List<Tsubuyaki> list = ts.searchTsubuyaki(form.getKeyword()); //検索文字を含むつぶやきを取得
        System.out.println(list);
        model.addAttribute("kensakuList", list);   //モデル属性にリストをセット
        model.addAttribute("kensakuForm", new TsubuyakiForm());  //空フォームをセット  
